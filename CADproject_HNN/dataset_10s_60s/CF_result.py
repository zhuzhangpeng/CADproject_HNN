import numpy as np
realdata = np.array(np.loadtxt('./dataset/CF_original.txt'))   #original dataset
predict = np.array(np.loadtxt('./CF_predict/test_predict.txt'))    #predict labels
...
print('predict result:\n total CAD cell:{0:.2f}, avg CAD cell:{1:.2f}, total time:{2:.2f}, '
      'avg time:{3:.2f}, predict accuracy:{4:.4f}'.format(precell, precell/predict.shape[0], pretime, pretime/predict.shape[0]
                                , precount/predict.shape[0]))
