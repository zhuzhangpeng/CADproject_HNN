...
model_path = 'CF_model'   #Save path of the model
model_name = 'dense_model'   #The prefix name of the model file
predict_name = 'CF_predict'   #Save path of prediction results
traindata = np.array(np.loadtxt('./dataset/CF_train.txt'))   #Read training set data
X_train = traindata[:, 0:num_F]   #Features of the training set
Y_train = traindata[:, num_F:]    #Label of training set

validdata = np.array(np.loadtxt('./dataset/CF_valid.txt'))    #Read validation set data
