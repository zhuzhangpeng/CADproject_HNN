...
testdata = np.loadtxt('dataset/CF_test.txt')      #Read test set data
...
ckpt = tf.train.get_checkpoint_state('./CF_model')
print(ckpt)
new_saver = tf.train.import_meta_graph('./CF_model/dense_model-500.meta')
new_saver.restore(sess, save_path='./CF_model/dense_model-500')
...
np.savetxt('./CF_predict/test_predict.txt', y,fmt='%.2f')   #Save prediction results
