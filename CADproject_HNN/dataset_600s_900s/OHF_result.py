import numpy as np
realdata = np.array(np.loadtxt('./dataset/OHF_original.txt'))   #original dataset
predict = np.array(np.loadtxt('./OHF_predict/test_predict.txt'))    #predict labels
...
c = 40   # feature count
d = 46   #feature count + orders count
...
svoout = 0    #SVO timeout count
for i in range(realdata.shape[0]//6*5, realdata.shape[0]):
    svolabel = int(realdata[i,d+6])-1
    if realdata[i, d+svolabel]==min(realdata[i, d:d+6]):
        svocount +=1
    if realdata[i, d+svolabel]==1800:
        svoout += 1
    svocell += realdata[i, c+svolabel]
    svot += realdata[i, d+svolabel]
print('SVO result:\n total CAD cell:{0:.2f}, avg CAD cell:{1:.2f}, total time:{2:.2f}, '
      'avg time:{3:.2f}, SVO accuracy: {4:.4f}, SVO timeout:{5}'.
      format(svocell, svocell/n, svot, svot/n, svocount/n, svoout))
