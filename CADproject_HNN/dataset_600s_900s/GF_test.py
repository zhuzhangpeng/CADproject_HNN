...
f = 30  #Number of feature
testdata = np.loadtxt('dataset/GF_test.txt')      #Read test set data
...
ckpt = tf.train.get_checkpoint_state('./GF_model')
print(ckpt)
new_saver = tf.train.import_meta_graph('./GF_model/dense_model-500.meta')
new_saver.restore(sess, save_path='./GF_model/dense_model-500')
...
np.savetxt('./GF_predict/test_predict.txt', y,fmt='%.2f')   #Save prediction results
